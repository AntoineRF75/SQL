...
-- USE AS (ALIAS)
-- WHERE
SELECT ROUND (AVG(salaire)) FROM employes AS salaire_moyen WHERE service = "informatique";
...
SELECT (SELECT COUNT(*) FROM employes WHERE sexe = "m") AS nb_homme, (SELECT COUNT(*) AS femme FROM employes WHERE sexe = "f") AS nb_femme;
-- OU --
...
SELECT service, COUNT(service) AS nb_employes FROM employes GROUP BY service;
...
-- GROUP BY
-- HAVING
SELECT service, COUNT(service) AS nb_employes FROM employes GROUP BY service HAVING COUNT(service) > 2;
